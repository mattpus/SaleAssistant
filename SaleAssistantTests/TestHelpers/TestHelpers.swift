...
func loadFixture(named name: String,
                 ...
                 line: UInt = #line) -> Data {
    let directory = URL(fileURLWithPath: String(describing: file)).deletingLastPathComponent().deletingLastPathComponent()
    
    // Prefer files inside a `MockResponses` subfolder next to the calling file
    let mockResponsesDir = directory.appendingPathComponent("MockResponses", isDirectory: true)
    let mockResponsesURL = mockResponsesDir.appendingPathComponent("\(name).\(fileExtension)")
